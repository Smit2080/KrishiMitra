...
import sampleProducts, { sampleFarmers } from './sampleProducts'
...
function Marketplace({ user }) {
  ...
  const fetchProducts = async () => {
    ...
    if (data && data.length > 0) {
      ...
      // Show sample products when database is empty
  ...
  const placeSingleOrder = async ({ product, qty, method }) => {
    // For sample/demo products, simulate order placement
  ...
  const placeCartOrders = async (method = 'online') => {
    ...
    // Separate real and sample items
    const realItems = cart.filter(item => !item.id?.startsWith('sample-'))
    const sampleItems = cart.filter(item => item.id?.startsWith('sample-'))
    
    let orderCount = sampleItems.length
    let firstOrderId = sampleItems.length > 0 ? 'demo-' + Date.now() : null

    if (realItems.length > 0) {
      const inserts = realItems.map(item => ({
        consumer_id: user.id,
      ...
      if (!error) {
        ...
        firstOrderId = firstOrderId || data?.[0]?.id
      }
    }
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (!scriptLoaded) throw new Error('Payment gateway unavailable. Try Cash on Delivery.')
      ...
      if (!checkoutRes.ok) throw new Error(checkoutData?.error || 'Payment server not available. Use Cash on Delivery.')
      ...
      // If online payment fails, auto-switch to COD and place order
      setPaymentMethod('cod')
      await placeCartOrders('cod')
      setOrdered('✅ Order placed with Cash on Delivery (Online payment unavailable)')
      setTimeout(() => setOrdered(null), 5000)
    } finally {
      setCheckoutLoading(false)
    }
  }
  ...
  const handleBuyNowCheckout = async () => {
    ...
    try {
      ...
      // Fallback to COD if online payment fails
      const createdOrder = await placeSingleOrder({ product: buyNowItem, qty: buyNowItem.qty, method: 'cod' })
      setReceipt({
        type: 'single',
        orderId: createdOrder?.id,
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6 py-3 flex flex-wrap gap-1.5 sm:gap-3 items-center justify-end border-b border-green-100">
        ...
        <button
          onClick={() => {
            ...
            if (usingSamples) { alert('Messages are available when real farmers list products. Sign up as a farmer to start selling!'); return }
            setShowChat(true)
          }}
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="flex gap-2 flex-wrap">
            {['All', 'Vegetables', 'Fruits', 'Grains', 'Dairy', 'Spices', 'Seeds'].map(cat => (
              <button
                key={cat}
        ...
        {loading ? (
          ...
          <>
            {usingSamples && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 mb-6 text-center">
                <p className="text-yellow-800 text-sm font-medium">
                  🌟 Showing sample products. Sign up as a farmer to list your own produce!
                </p>
              </div>
            )}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {filtered.map(p => (
              <div key={p.id} className="bg-white rounded-2xl shadow hover:shadow-lg transition p-5 flex flex-col">
                ...
                {/* Product Image */}
                <div
                  className="bg-green-50 rounded-xl h-40 flex items-center justify-center mb-4 cursor-pointer hover:opacity-90 transition overflow-hidden"
                  onClick={() => setSelectedProduct(p)}
                >
                  {p.image_url ? (
                    <img src={p.image_url} alt={p.name} className="w-full h-full object-cover rounded-xl" />
                  ) : (
                    <span className="text-5xl">{getCategoryEmoji(p.category)}</span>
                  )}
                </div>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 z-10">
            ...
            <div className="bg-green-100 rounded-xl h-48 flex items-center justify-center mb-5 overflow-hidden">
              {selectedProduct.image_url ? (
                <img src={selectedProduct.image_url} alt={selectedProduct.name} className="w-full h-full object-cover rounded-xl" />
              ) : (
                <span className="text-7xl">{getCategoryEmoji(selectedProduct.category)}</span>
              )}
            </div>
